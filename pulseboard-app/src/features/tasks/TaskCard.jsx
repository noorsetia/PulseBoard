...
export const TaskCard = ({ task }) => {
  ...
  return (
    <Card hover className="border-l-4 dark:border-l-4" style={{ borderLeftColor: statusConfig.color.includes('yellow') ? '#f59e0b' : statusConfig.color.includes('blue') ? '#3b82f6' : '#10b981' }}>
      <div className="flex justify-between items-start mb-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`px-3 py-1 rounded text-sm font-medium ${priorityConfig.color}`}>
          ...
          {task.tags && task.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-2">
              {task.tags.map(tagId => {
                ...
                return tag ? (
                  <span key={tagId} className={`px-2.5 py-1 rounded-full text-sm ${tag.color}`}>
                    {tag.label}
                  </span>
                ) : null;
              })}
      ...
      {task.description && (
        <p className="text-base text-gray-600 dark:text-gray-400 mb-4">{task.description}</p>
      )}
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center space-x-3">
          ...
          {task.dueDate && (
            <span className={`text-sm flex items-center gap-1 ${dueDateColor(task.dueDate)}`}>
        ...
        <div className="flex space-x-2">
          {availableTransitions.map((status) => (
            <button
              ...
              className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${TASK_STATUS_CONFIG[status].color} ${TASK_STATUS_CONFIG[status].hoverColor}`}
